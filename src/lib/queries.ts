...
// ==========================================
// 1. EJERCICIOS (CATÁLOGO E HISTORIAL)
// ==========================================
...
export async function fetchExerciseHistory(exerciseId: string) {
  const { data, error } = await supabase
    ...
    .select(`
      id, start_time, routine_id,
      workout_sets!inner ( id, weight, reps, unit, set_type, rir, is_completed )
    `)
...
export async function createExercise(exerciseData: Partial<Exercise>) {
  ...
  const { data, error } = await supabase.from('exercises').insert({ ...exerciseData, user_id: user.id }).select().single()
  if (error) throw error
  return data
}
...
export async function deleteAllExercises() {
  const { error } = await supabase.from('exercises').delete().not('id', 'is', null) 
  if (error) throw error
}
...
async function fetchExistingPrRecords(exerciseIds: string[]): Promise<ExistingPrRecord[]> {
  ...
  const { data, error } = await supabase.from('personal_records').select('exercise_id, record_type, value').in('exercise_id', exerciseIds)
  if (error) { if (isMissingTableError(error)) return []; throw error; }
  return (data ?? []) as ExistingPrRecord[]
}

async function savePrEvents(userId: string, sessionId: string, candidates: ExistingPrRecord[]) {
  if (candidates.length === 0) return
  const recordsPayload = candidates.map((c) => ({ user_id: userId, exercise_id: c.exercise_id, record_type: c.record_type, value: c.value, session_id: sessionId }))
  const { error: recordsError } = await supabase.from('personal_records').upsert(recordsPayload, { onConflict: 'user_id,exercise_id,record_type' })
  if (recordsError && !isMissingTableError(recordsError)) throw recordsError
  const { error: eventsError } = await supabase.from('feed_events').insert(candidates.map((c) => ({ user_id: userId, session_id: sessionId, event_type: 'pr_achieved', payload: c })))
...
export async function finishWorkoutSession({ startTime, endTime, workoutExercises, sessionNotes, sessionOptions }: FinishWorkoutInput): Promise<void> {
  ...
  const { error: setsError } = await supabase.from('workout_sets').insert(allSetsToInsert)
...
export async function updateRoutine(routineId: string, name: string, notes: string, folder: string | null, days: any[]) {
  const { error: updateErr } = await supabase.from('routines').update({ name, notes, folder: folder || null }).eq('id', routineId);
  ...
  for (let i = 0; i < days.length; i++) {
    const day = days[i];
    const { data: newDay, error: dayErr } = await supabase.from('routine_days').insert({ routine_id: routineId, name: day.name, day_order: day.day_order }).select('id').single();
    if (dayErr) throw dayErr;

    if (day.exercises.length > 0) {
      const exPayload = day.exercises.map((ex: any) => ({
        routine_day_id: newDay.id, exercise_id: ex.exercise_id, target_sets: ex.target_sets, target_reps: ex.target_reps, config: ex.config,
      }));
...
export async function cloneRoutine(routineId: string) {
  ...
  for (const day of original.routine_days) {
    const { data: newDay, error: dayErr } = await supabase.from('routine_days').insert({ routine_id: newRoutine.id, name: day.name, day_order: day.day_order, config: day.config }).select('id').single()
    if (dayErr) throw dayErr

    const exercisesPayload = (day.routine_exercises ?? []).map((ex) => ({
      routine_day_id: newDay.id, exercise_id: ex.exercise_id, target_sets: ex.target_sets, target_reps: ex.target_reps, superset_id: ex.superset_id, set_type: ex.set_type, pr_mode: ex.pr_mode, pr_fixed_weight: ex.pr_fixed_weight, config: ex.config
    }))
    if (exercisesPayload.length > 0) {
      const { error: exercisesErr } = await supabase.from('routine_exercises').insert(exercisesPayload)
...
export async function createRoutine(name: string, notes: string, exercises: { exercise_id: string; sets: number; reps: number | null }[]) {
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Usuario no autenticado')

  const { data: routine, error: routineErr } = await supabase.from('routines').insert({ user_id: user.id, name, notes }).select('id').single()
  if (routineErr) throw routineErr

  const { data: day, error: dayErr } = await supabase.from('routine_days').insert({ routine_id: routine.id, name: 'Día 1', day_order: 1 }).select('id').single()
  if (dayErr) throw dayErr

  const exercisesPayload = exercises.map((ex) => ({ routine_day_id: day.id, exercise_id: ex.exercise_id, target_sets: ex.sets, target_reps: ex.reps }))
  if (exercisesPayload.length > 0) {
    const { error: exercisesErr } = await supabase.from('routine_exercises').insert(exercisesPayload)
    if (exercisesErr) throw exercisesErr
  }
...
export async function createStructuredRoutine(name: string, folder: string, notes: string, days: { name: string; exercises: { exercise_id: string; target_sets: number; target_reps: number; config: any; }[]; }[]) {
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Usuario no autenticado')

  const { data: routine, error: routineErr } = await supabase.from('routines').insert({ user_id: user.id, name, folder: folder || null, notes }).select('id').single()
  if (routineErr) throw routineErr

  for (let i = 0; i < days.length; i++) {
    const day = days[i]
    const { data: newDay, error: dayErr } = await supabase.from('routine_days').insert({ routine_id: routine.id, name: day.name, day_order: i + 1 }).select('id').single()
    if (dayErr) throw dayErr

    if (day.exercises.length > 0) {
      const exPayload = day.exercises.map((ex) => ({ routine_day_id: newDay.id, exercise_id: ex.exercise_id, target_sets: ex.target_sets, target_reps: ex.target_reps, config: ex.config }))
      const { error: exErr } = await supabase.from('routine_exercises').insert(exPayload)
      if (exErr) throw exErr
    }
  }
...
export async function deleteAllWorkoutHistory() {
  const { data: { user } } = await supabase.auth.getUser()
  if (!user) throw new Error('Usuario no autenticado')
  const { data: sessions, error: fetchErr } = await supabase.from('workout_sessions').select('id').eq('user_id', user.id)
  if (fetchErr) throw fetchErr

  if (sessions && sessions.length > 0) {
    const sessionIds = sessions.map(s => s.id)
    const { error: setsErr } = await supabase.from('workout_sets').delete().in('session_id', sessionIds)
    if (setsErr) throw setsErr
    const { error: sessionErr } = await supabase.from('workout_sessions').delete().in('id', sessionIds)
